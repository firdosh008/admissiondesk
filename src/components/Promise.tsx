export function EditorialPromise() {
  return (
    <section className="section-light">
      <div className="container-x py-12 md:py-24 lg:py-32">
        <div className="grid lg:grid-cols-12 gap-8 md:gap-12 items-start">
          {/* ── Left: Editorial aside ── */}
          <aside className="lg:col-span-4">
            <p className="eyebrow">Our counselling promise</p>
            <h2 className="font-display text-3xl sm:text-4xl md:text-5xl mt-4 leading-[1.05] text-[color:var(--forest-deep)]">
              We work for the
              <br />
              <span className="font-italic-serif">student.</span>
            </h2>
            <span className="rule-ornament mt-6">est. 2018</span>
          </aside>

          {/* ── Right: Editorial copy ── */}
          <div className="lg:col-span-8 lg:pl-12 lg:border-l lg:border-[color:var(--rule-soft)] columns-1 lg:columns-2 gap-10 text-[color:var(--ink-soft)] leading-[1.8]">
            <p className="dropcap text-[1.06rem] mb-6">
              Choosing a university is, for most families in India, a serious
              financial and emotional decision. It deserves more than a glossy
              brochure and a follow-up call.
            </p>
            <p className="text-[1.04rem] mb-6">
              admissiondesk was founded on a simple premise: students deserve
              clear counselling before they choose a course, campus, fee plan,
              or scholarship route.
            </p>
            <p className="text-[1.04rem] mb-6">
              Our active counselling pages currently focus on{" "}
              <span className="font-display italic text-[color:var(--forest-deep)]">
                Uttaranchal University
              </span>{" "}
              and{" "}
              <span className="font-display italic text-[color:var(--forest-deep)]">
                Graphic Era
              </span>
              . Each institution carries a different academic identity and a
              different placement profile. Counselling is the work of telling
              you which option matches your goals — and which does not.
            </p>
            <p className="text-[1.04rem]">
              We do this work in plain language, in your local language if
              you&apos;d like, and on your timeline. The phone call is free.
              The advice is honest. The decision, always, is yours.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}
